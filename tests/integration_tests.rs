    ...
    rust_parallel()
        ...
        .stdout(
            (predicate::str::contains("\n").count(3))
                .and(
                    predicate::str::contains(
                        r#"cmd="/bin/sh",args=["-c", "echo A"],line=command_line_args:1"#,
                    )
                    .count(1),
                )
                .and(
                    predicate::str::contains(
                        r#"cmd="/bin/sh",args=["-c", "echo B"],line=command_line_args:2"#,
                    )
                    .count(1),
                )
                .and(
                    predicate::str::contains(
                        r#"cmd="/bin/sh",args=["-c", "echo C"],line=command_line_args:3"#,
                    )
                    .count(1),
                ),
        )
